...
void pddpana::GetStoppingParticles::analyze(art::Event const& e)
{
  ...
  for (unsigned trk = 0; trk < Tracks->size(); ++trk) {
    ...
    if(track.Start().Y() - fAVy_min < fFiducialCut || track.Start().Y() - fAVy_min > 100 - fFiducialCut) is_out = true;
    if(track.Start().Z() - fAVz_min < fFiducialCut || track.Start().Z() - fAVz_min > 100 - fFiducialCut) is_out = true;

    if(!is_out){
      // Filter out tracks that e (unknown T0)
      if(track.End().Y() - fAVy_min < fFiducialCut || track.End().Y() - fAVy_min > 100 - fFiducialCut) is_out = true;
      if(track.End().Z() - fAVz_min < fFiducialCut || track.End().Z() - fAVz_min > 100 - fFiducialCut) is_out = true;
    }

    
    if(is_out){ ntrks[2]++; std::cout << "  " << track.Start().Y() << "," << track.End().Y() << " - " << track.Start().Z() << "," << track.End().Z()  << std::endl;  continue;}
